...
export const FIREBASE_CONFIG = {
  apiKey: 'AIzaSyBCxZAHctxyYQXufn25MDPSfJ034YtF9Us',
  authDomain: 'arcane-ai-os.firebaseapp.com',
  projectId: 'arcane-ai-os',
  storageBucket: 'arcane-ai-os.firebasestorage.app',
  messagingSenderId: '590368346743',
  appId: '1:590368346743:web:409c3dddea989d98baec72',
  // The console also hands out `measurementId`. That is Google Analytics,
  // which this deck does not use and which would pull a fourth SDK bundle
  // for nothing, so it is deliberately not here.
};
...
export function setFirebaseConfig(text) {
  ...
  // Once a config is committed it backfills every required field, which
  // would let any text at all through as "saved". A paste has to carry at
  // least one field this app recognises before it counts as a config.
  if (!Object.keys(next).length) return { ok: false, error: 'That is not a Firebase config.' };
  // A partial paste is still fine — the committed values fill the rest —
  // but between them the required fields have to be there.
